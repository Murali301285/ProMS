...
const config = {
    ...
    password: process.env.DB_PASSWORD || 'Chennai@42', // Fallback based on typical local setup, but preferably use env
    server: process.env.DB_SERVER || 'localhost',
    database: process.env.DB_NAME || 'ProMS_Dev',
    options: {
        encrypt: false,
...
async function testQuery() {
    try {
        console.log("Connecting to DB...", config.server);
        ...
        console.error("❌ SQL Error:", error.originalError?.info || error.message);
    } finally {
        process.exit();
    }
}
