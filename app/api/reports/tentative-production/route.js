...
export async function GET(req) {
    try {
        ...
        if (type === 'shifts') {
            const data = await executeQuery('SELECT SlNo, ShiftName FROM [Master].[TblShift] WHERE IsDelete = 0 AND IsActive = 1 ORDER BY ShiftName');
...
export async function POST(req) {
    ...
    try {
        ...
        const query = `
            DECLARE @Date DATE = @dateInput;
            DECLARE @ShiftId int = @shiftIdInput;
