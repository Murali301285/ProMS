...
export async function GET() {
    try {
        ...
        // Hardcoded query from recent-list for testing
        // Removed User ID filter to see if ANY data exists
        const query = `
            SELECT TOP 20 
                T.SlNo, 
        ...
        return NextResponse.json({ success: true, count: result.recordset.length, data: result.recordset });

    } catch (error) {
        return NextResponse.json({ success: false, error: error.message, originalError: error.originalError?.info }, { status: 500 });
    }
}
