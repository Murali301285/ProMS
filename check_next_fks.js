...
async function checkFK() {
    try {
        ...
        const tables = ['TblMaterialRehandling', 'TblEquipmentReading'];
        ...
        for (const table of tables) {
            ...
            const query = `
                ...
                WHERE 
                    tp.name = '${table}' AND (cp.name = 'CreatedBy' OR cp.name = 'UpdatedBy');
            `;

            const res = await sql.query(query);
            if (res.recordset.length === 0) {
                console.log(`No Foreign Keys found for CreatedBy/UpdatedBy on ${table}.`);
            } else {
