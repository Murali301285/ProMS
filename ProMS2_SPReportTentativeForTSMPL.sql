CREATE OR ALTER PROCEDURE [dbo].[ProMS2_SPReportTentativeForTSMPL] 
    @Date DATE = NULL,
    @ShiftId int = NULL,
	@ShiftChange int = 0,
	@BreakTime int = 0,
...
BEGIN
		...
		Declare @ProdCoal decimal(18,2)=0,@ProdOB decimal(18,2)=0,@WPCoalQty decimal(18,2)=0,@WPObQty decimal(18,2)=0,@CarpettingObQty decimal(18,2)=0,@RehandlingCoalQty decimal(18,2)=0;
		--Production Quantity
        select @ProdCoal=Convert(decimal(18,2), ISNULL(SUM(A.RomCoal),0.00)) from (
		select  T1.EquipmentGroupId,T2.Name as EquipmentGroup,T0.MaterialId,SUM(T0.NoofTrip*T0.QtyTrip) AS RomCoal
		...
		WITH WasteHandlingTable As( select  T1.EquipmentGroupId,T2.Name as EquipmentGroup,T3.Name as Scale,T0.MaterialId,SUM(T0.NoofTrip) AS NoofTrip,T0.QtyTrip
		from Trans.TblLoading T0
		Join Master.TblEquipment T1 on T1.SlNo=T0.HaulerEquipmentId
		Join Master.TblEquipmentGroup T2 on T2.SlNo=T1.EquipmentGroupId
		...
		Outer apply (select SUM(NoofTrip*QtyTrip) as OverBurden from WasteHandlingTable where MaterialId=@OBId and EquipmentGroupId=T0.EquipmentGroupId) T1
		Outer apply (select SUM(NoofTrip*QtyTrip) as TopSoil from WasteHandlingTable where MaterialId=@TopSoilId and EquipmentGroupId=T0.EquipmentGroupId) T2
		...
		from WP3Table T0
		Outer apply (select SUM(NoofTrip*QtyTrip) as OverBurden from WP3Table where MaterialId=@OBId and EquipmentGroupId=T0.EquipmentGroupId) T1
		Outer apply (select SUM(NoofTrip*QtyTrip) as TopSoil from WP3Table where MaterialId=@TopSoilId and EquipmentGroupId=T0.EquipmentGroupId) T2
		Outer apply (select SUM(NoofTrip*QtyTrip) as Coal from WP3Table where MaterialId=@RomCoalId and EquipmentGroupId=T0.EquipmentGroupId) T3
		group by T0.EquipmentGroupId,T0.EquipmentGroup,T0.Scale,T1.OverBurden,T2.TopSoil,T3.Coal ) A;

		--Carpetting Quantity
		select @CarpettingObQty=Convert(decimal(18,2),ISNULL(SUM(OBQty),0.00)) from (
		select  T1.EquipmentGroupId,T2.Name as EquipmentGroup,SUM(T0.NoofTrip*T0.QtyTrip) as OBQty
		...
		Join Master.TblEquipment T1 on T1.SlNo=T0.HaulerEquipmentId
		Join Master.TblEquipmentGroup T2 on T2.SlNo=T1.EquipmentGroupId
		Where T0.IsDelete=0 and T0.DestinationId=@DestinationCarpetingWorkId and Convert(date,T0.LoadingDate)=@Date and T0.ShiftId=@ShiftId
		group by T1.EquipmentGroupId,T2.Name,T0.QtyTrip
		) A;

		--Coal Rehandling
		select @RehandlingCoalQty=Convert(decimal(18,2),ISNULL(SUM(A.CoalQty),0.00)) from (
		select  T1.EquipmentGroupId,T2.Name as EquipmentGroup,SUM(T0.NoofTrip*T0.QtyTrip) AS CoalQty
		from Trans.TblMaterialRehandling T0
		Join Master.TblEquipment T1 on T1.SlNo=T0.HaulerEquipmentId
		Join Master.TblEquipmentGroup T2 on T2.SlNo=T1.EquipmentGroupId
		Where T0.IsDelete=0 and T0.MaterialId in (@RomCoalId) and Convert(date,T0.RehandlingDate)=@Date and T0.ShiftId=@ShiftId
		group by T1.EquipmentGroupId,T2.Name) A;

		select @ProdCoal as ProdCoal,@ProdOB as ProdOB,@WPCoalQty as WPCoalQty,@WPObQty as WPObQty,@RehandlingCoalQty as RehandlingCoalQty,@CarpettingObQty as CarpettingObQty,
		@ShiftChange as ShiftChange,@BreakTime as BreakTime,@Blasting as Blasting,@Others as Others,(@ShiftChange+@BreakTime+@Blasting+@Others) as Totalmin,
		Convert(decimal(18,1),(Convert(decimal(18,2),@ShiftChange)/60)) as TotalShiftChangeHrs,Convert(decimal(18,1),(Convert(decimal(18,2),@BreakTime)/60)) as TotalBreakTimeHrs,
		Convert(decimal(18,1),(Convert(decimal(18,2),@Blasting)/60)) as TotalBlastingHrs,Convert(decimal(18,1),(Convert(decimal(18,2),@Others)/60)) as TotalOthersHrs,
		Convert(decimal(18,1),(Convert(decimal(18,2),(@ShiftChange+@BreakTime+@Blasting+@Others))/60)) as TotalHrs,
		(8-Convert(decimal(18,1),(Convert(decimal(18,2),(@ShiftChange+@BreakTime+@Blasting+@Others))/60))) as TotalWorkingHrs,
		
		CASE WHEN (8-Convert(decimal(18,1),(Convert(decimal(18,2),(@ShiftChange+@BreakTime+@Blasting+@Others))/60))) > 0 
		THEN Convert(decimal(18,2),(@ProdCoal/(8-Convert(decimal(18,1),(Convert(decimal(18,2),(@ShiftChange+@BreakTime+@Blasting+@Others))/60))))) 
		ELSE 0 END as ProdCoalPerHrs,
		
		CASE WHEN (8-Convert(decimal(18,1),(Convert(decimal(18,2),(@ShiftChange+@BreakTime+@Blasting+@Others))/60))) > 0 
		THEN Convert(decimal(18,2),(@ProdOB/(8-Convert(decimal(18,1),(Convert(decimal(18,2),(@ShiftChange+@BreakTime+@Blasting+@Others))/60))))) 
		ELSE 0 END as ProdOBPerHrs


		--Crusher Details
		select  T1.Name as Plant,SUM(T0.RunningHr) as RunningHr,SUM(T0.TotalQty) as TotalQty 
