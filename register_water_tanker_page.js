...
async function run() {
    try {
        ...
        // 1. Get Transaction Module
        // Provide backup name "Transactions" just in case
        const mod = await pool.request().query("SELECT SlNo FROM [Master].[TblModule] WHERE ModuleName IN ('Transaction', 'Transactions')");
        ...
        const page = { name: 'Water Tanker Entry', path: '/dashboard/transaction/water-tanker-entry' };
        ...
        if (!pageId) {
            console.log(`Inserting ${page.name}...`);
